...
import {
  buildSupertonicSidecar,
  copyBuiltSupertonicSidecar,
  pyinstallerOutputPath,
  releaseModeFromArgs,
  targetTripleFromArgs,
  venvPythonPath,
} from "./build-supertonic-sidecar.js";

describe("Supertonic sidecar build script", () => {
  ...
  it("uses the PyInstaller onefile output name for the target", () => {
    ...
    ).toMatch(/openbrief-supertonic\.exe$/);
  });

  it("copies built sidecars to the Tauri target-triple name", () => {
    const root = mkdtempSync(join(tmpdir(), "openbrief-supertonic-copy-"));
    const sourcePath = join(root, "openbrief-supertonic");
    const binariesDir = join(root, "binaries");
    writeFileSync(sourcePath, "#!/bin/sh\n");

    const result = copyBuiltSupertonicSidecar({
      sourcePath,
      binariesDir,
      targetTriple: "aarch64-apple-darwin",
    });

    expect(result.destinationName).toBe("openbrief-supertonic-aarch64-apple-darwin");
    expect(existsSync(result.destinationPath)).toBe(true);
  });

  it("creates a placeholder and skips PyInstaller in debug mode", () => {
    const root = mkdtempSync(join(tmpdir(), "openbrief-supertonic-build-"));
    ...
    expect(
      existsSync(join(root, "binaries", "openbrief-supertonic-x86_64-unknown-linux-gnu")),
    ).toBe(true);
  });

  it("rejects cross-target PyInstaller release builds", () => {
    const root = mkdtempSync(join(tmpdir(), "openbrief-supertonic-cross-"));
